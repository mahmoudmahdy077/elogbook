      ...
      {(action || secondaryAction) && (
        <div className="flex items-center justify-center gap-3 pt-1">
          ...
          {secondaryAction && (secondaryAction.href ? (
            <a href={secondaryAction.href} className="inline-flex min-h-[44px] items-center px-4 py-2 rounded-lg border border-border text-sm font-medium text-text-secondary hover:border-primary hover:text-primary transition-colors">
              {secondaryAction.label}
            </a>
          ) : (
            <button onClick={secondaryAction.onClick} className="inline-flex min-h-[44px] items-center px-4 py-2 rounded-lg border border-border text-sm font-medium text-text-secondary hover:border-primary hover:text-primary transition-colors">
              {secondaryAction.label}
            </button>
          ))}
        </div>
