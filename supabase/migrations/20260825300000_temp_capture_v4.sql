-- Capture v5: two-phase, statement+row capture triggers, inline attempts
CREATE OR REPLACE FUNCTION public.dbg_cap_deleted()
RETURNS TRIGGER LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  BEGIN
    INSERT INTO public._swarm_debug_results VALUES ((SELECT COALESCE(MAX(line),80)+1 FROM public._swarm_debug_results),
      jsonb_build_object('phase',TG_LEVEL,'op',TG_OP,'OLD.d',OLD.deleted_at,'NEW.d',NEW.deleted_at));
  EXCEPTION WHEN OTHERS THEN NULL;
  END;
  RETURN NEW;
END $$;

DROP TRIGGER IF EXISTS trg_dbg_row ON public.case_entries;
DROP TRIGGER IF EXISTS trg_dbg_stmt ON public.case_entries;
CREATE TRIGGER trg_dbg_row BEFORE UPDATE ON public.case_entries
FOR EACH ROW EXECUTE FUNCTION public.dbg_cap_deleted();
CREATE TRIGGER trg_dbg_stmt BEFORE UPDATE ON public.case_entries
FOR EACH STATEMENT EXECUTE FUNCTION public.dbg_cap_deleted();

DO $$
DECLARE
  v_uid UUID := 'f2a0d3a0-b3a0-4026-bdfa-ba0a4688a783';
  v_tenant UUID := '9cd50d60-febe-4adf-be0f-a36bf82762f6';
  v_claims JSONB := jsonb_build_object('aud','authenticated','role','authenticated','sub',v_uid,
    'app_metadata', jsonb_build_object('tenant_id',v_tenant,'user_role','resident'));
  v_pid UUID; v_tmpl UUID; v_id UUID;
  v_keep JSONB;
  r RECORD;
BEGIN
  DELETE FROM public._swarm_debug_results WHERE line >= 80;

  PERFORM set_config('role','authenticated', true);
  PERFORM set_config('request.jwt.claims', v_claims::text, true);
  SELECT id INTO v_pid FROM public.profiles WHERE user_id = auth.uid();
  SELECT id INTO v_tmpl FROM public.case_templates LIMIT 1;
  PERFORM set_config('role','postgres', true);

  -- stash + wipe UPDATE policies (postgres)
  SELECT jsonb_agg(jsonb_build_object('name',policyname,'roles',roles,'qual',qual,'wc',with_check))
    INTO v_keep FROM pg_policies
   WHERE schemaname='public' AND tablename='case_entries' AND cmd='UPDATE';
  FOR r IN SELECT policyname FROM pg_policies
           WHERE schemaname='public' AND tablename='case_entries' AND cmd='UPDATE'
  LOOP EXECUTE format('DROP POLICY %I ON public.case_entries', r.policyname); END LOOP;

  -- ── X1: bare true/true control ──
  EXECUTE 'CREATE POLICY p_v ON public.case_entries FOR UPDATE TO authenticated USING (deleted_at IS NULL) WITH CHECK (true)';

  PERFORM set_config('role','authenticated', true);
  PERFORM set_config('request.jwt.claims', v_claims::text, true);
  IF v_pid IS NOT NULL THEN
    INSERT INTO public.case_entries (tenant_id,resident_id,template_id,case_date,field_values,status,accreditation_mappings,is_deidentified,patient_mrn,patient_dob,patient_age_years,patient_hash)
  VALUES (v_tenant,v_pid,v_tmpl,CURRENT_DATE,jsonb_build_object('procedure_name','x1'),'draft','[]'::jsonb,TRUE,NULL,NULL,NULL,'x') RETURNING id INTO v_id;
  END IF;
  BEGIN
    UPDATE public.case_entries SET deleted_at = NOW() WHERE id = v_id;
    INSERT INTO public._swarm_debug_results VALUES (91, jsonb_build_object('x1-control-wc-true','OK'));
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO public._swarm_debug_results VALUES (91, jsonb_build_object('x1-control-wc-true','FAIL','msg',SQLERRM));
  END;

  -- ── X2: WC = deleted_at IS NOT NULL ──
  RESET ROLE;
  IF v_pid IS NOT NULL THEN
    INSERT INTO public.case_entries (tenant_id,resident_id,template_id,case_date,field_values,status,accreditation_mappings,is_deidentified,patient_mrn,patient_dob,patient_age_years,patient_hash)
  VALUES (v_tenant,v_pid,v_tmpl,CURRENT_DATE,jsonb_build_object('procedure_name','x2'),'draft','[]'::jsonb,TRUE,NULL,NULL,NULL,'x') RETURNING id INTO v_id;
  END IF;
  EXECUTE 'DROP POLICY p_v ON public.case_entries';
  EXECUTE 'CREATE POLICY p_v ON public.case_entries FOR UPDATE TO authenticated USING (deleted_at IS NULL) WITH CHECK (deleted_at IS NOT NULL)';

  PERFORM set_config('role','authenticated', true);
  PERFORM set_config('request.jwt.claims', v_claims::text, true);
  BEGIN
    UPDATE public.case_entries SET deleted_at = NOW() WHERE id = v_id;
    INSERT INTO public._swarm_debug_results VALUES (92, jsonb_build_object('x2-deleted-notnull','OK'));
  EXCEPTION WHEN OTHERS THEN
    INSERT INTO public._swarm_debug_results VALUES (92, jsonb_build_object('x2-deleted-notnull','FAIL','msg',SQLERRM));
  END;

  RESET ROLE;

  -- restore original policies + cleanup probe rows
  FOR r IN SELECT * FROM jsonb_to_recordset(v_keep) AS x(name text, roles jsonb, qual text, wc text)
  LOOP
    EXECUTE format($p$CREATE POLICY %I ON public.case_entries FOR UPDATE TO authenticated USING (%s) WITH CHECK (%s)$p$, r.name, r.qual, r.wc);
  END LOOP;
END $$;
