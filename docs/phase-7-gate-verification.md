...
| 6 | Trivy container scan | ✅ (workflow) | `.github/workflows/container-scan.yml` builds `apps/web/Dockerfile` and runs `aquasecurity/trivy-action@v0.36.0` with `--severity CRITICAL --exit-code 1`. Docker daemon was not reachable in this worktree, so the build+scan is a CI check. |
