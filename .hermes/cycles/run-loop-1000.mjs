...
const STATE_FILE = new globalThis.URL('./loop-state-1000.json', import.meta.url);
...
console.log(`[loop] ${done}/1000 cycles already complete`);
...
for (const suite of ROTATION) {
  ...
  const t0 = Date.now();
  // Self-heal demo quota before approvals suite (it inserts new cases; free plan caps at 20)
  if (suite === 'cycle3-approvals') {
    try {
      execFileSync(process.execPath, [fileURLToPath(new globalThis.URL('./free-quota-slots.mjs', import.meta.url))], {
        encoding: 'utf8', timeout: 60000, stdio: ['ignore', 'pipe', 'pipe'],
      });
    } catch { /* cleanup best-effort; suite retry handles transient state */ }
  }
