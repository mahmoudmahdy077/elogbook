...
const GLOBAL_TEMPLATE_ID = "00000000-0000-4000-8000-000000000010";
...
// evaluations list (faculty_evaluations is the app's evaluations table)
await bench("evaluations_list_faculty", () =>
  fetch(`${rest}/faculty_evaluations?select=id,status,created_at&order=created_at.desc&limit=20`, { headers: H })
);

// notifications list with correct columns
await bench("notifications_list_fixed", () =>
  fetch(`${rest}/notifications?select=id,type,title,read_at,created_at&user_id=eq.${userId}&order=created_at.desc&limit=20`, { headers: H })
);

// template visibility for resident
let templateId = GLOBAL_TEMPLATE_ID;
{
  const r = await fetch(`${rest}/case_templates?select=id,name&limit=5`, { headers: H });
  const arr = await r.json();
  console.log("templates visible to resident:", JSON.stringify(arr).slice(0, 300));
  if (Array.isArray(arr) && arr.length > 0) templateId = arr[0].id;
}
results.template_used = templateId;
...
// verify rows exist then tombstone ALL created rows
{
  const v = await fetch(`${rest}/case_entries?id=in.(${ids.join(",")})&select=id,deleted_at`, { headers: H });
  ...
  const vb = await tb._res?.text?.();
  console.log(`tombstone: status=${tb.status} in ${tb.ms.toFixed(0)}ms`);
  // verify tombstoned
  const v2 = await fetch(`${rest}/case_entries?id=in.(${ids.join(",")})&select=id,deleted_at&deleted_at=not.is.null`, { headers: H });
  const arr2 = await v2.json();
  results.tombstoned_rows = Array.isArray(arr2) ? arr2.length : String(arr2).slice(0, 200);
  results.tombstone_status = tb.status;
  results.created_row_ids = ids;
}

writeFileSync(new URL("./perf_results_extra.json", import.meta.url), JSON.stringify(results, null, 2));
console.log("DONE");
